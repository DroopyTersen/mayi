...
describe("TurnMachine - LAY_DOWN command", () => {
  ...
  describe("successful lay down - Round 6 (1 set + 2 runs) - must use ALL cards", () => {
    it("accepts valid 1 set and 2 runs using ALL 12 cards and goes out immediately", () => {
      // In Round 6, laying down = going out (must use ALL cards)
      const threeC = card("3", "clubs");
      const threeD = card("3", "diamonds");
      const threeH = card("3", "hearts");
      const threeS = card("3", "spades"); // 4th card for extended set
      const fiveS = card("5", "spades");
      const sixS = card("6", "spades");
      const sevenS = card("7", "spades");
      const eightS = card("8", "spades");
      ...
      const tenH = card("10", "hearts");
      const jackH = card("J", "hearts");
      // The card drawn from stock to complete the hearts run
      const queenH = card("Q", "hearts");

      const input = {
        playerId: "player-1",
        roundNumber: 6 as const,
        // 11 cards - after drawing will have 12
        hand: [threeC, threeD, threeH, threeS, fiveS, sixS, sevenS, eightS, nineH, tenH, jackH],
        stock: [queenH], // Q♥ will complete the hearts run
        discard: [card("5", "clubs")],
        isDown: false,
        table: [],
      };
      ...
      const hand = actor.getSnapshot().context.hand;
      expect(hand.length).toBe(12); // 11 + 1 drawn

      // Lay down ALL 12 cards: 4-card set + 4-card run + 4-card run
      actor.send({
        type: "LAY_DOWN",
        melds: [
          { type: "set" as const, cardIds: [threeC.id, threeD.id, threeH.id, threeS.id] },
          { type: "run" as const, cardIds: [fiveS.id, sixS.id, sevenS.id, eightS.id] },
          { type: "run" as const, cardIds: [nineH.id, tenH.id, jackH.id, queenH.id] },
        ],
      });

      // In Round 6, laying down all cards = went out immediately
    ...
    it("rejects partial laydown in Round 6 - must use ALL cards", () => {
      ...
      const input = {
        ...createTurnInput(),
        roundNumber: 6 as const,
        hand: [threeC, threeD, threeH, fiveS, sixS, sevenS, eightS, nineH, tenH, jackH, queenH],
      };
      const actor = createActor(turnMachine, { input });
      actor.start();
      actor.send({ type: "DRAW_FROM_STOCK" });
      ...
      // Try to lay down only 11 cards (leaving 1 behind) - should be rejected
      actor.send({
        type: "LAY_DOWN",
        melds: [
          { type: "set" as const, cardIds: [threeC.id, threeD.id, threeH.id] },
      ...
      // Should still be in drawn state - laydown rejected
      expect(actor.getSnapshot().value).toBe("drawn");
      expect(actor.getSnapshot().context.hand.length).toBe(12); // No cards removed
      expect(actor.getSnapshot().context.lastError).toContain("Round 6");
    });
  });
